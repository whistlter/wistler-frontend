...
export const COMMUNITIES_DB: CommunitiesApi[] = Array.from(
    ...
    (_, i) => ({
        id: i + 1,
        community_Name: `Community ${i + 1}`,
        Members: `${Math.floor(Math.random() * 5000) + 100}`,
        Visibility: i % 3 === 0 ? "Private" : "Public",
        is_active: i % 5 !== 0,
        joined_at: new Date(Date.now() - i * 86_400_000 * 2).toISOString(),
    })
);
...
export const COMMUNITY_POSTS_DB: CommunitiesPostApi[] = Array.from(
    ...
    (_, i) => ({
        id: i + 1,
        caption: `This is a sample post caption for post #${i + 1}. It contains some interesting content.`,
        author: `Author ${i + 1}`,
        flags_count: Math.floor(Math.random() * 20),
        is_active: i % 10 !== 0,
