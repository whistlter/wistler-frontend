...
export function MomentPosts() {
    ...
    useEffect(() => {
        setPlaceholder('Search posts by content or user...');
        return () => clearSearch();
    ...
    const { data, isLoading, isError, error } = useMomentPosts(id || '', page, PAGE_SIZE, searchTerm, startDate, endDate, statusFilter);
    ...
    const posts = data?.payload?.comments ?? [];
    const rows: MomentPostRowDTO[] = posts.map(mapMomentPostToRowDTO);
    ...
    const columns: TableColumn<MomentPostRowDTO>[] = [
        ...
        { key: MOMENT_POST_VARIANTE.POST_CONTENT, header: 'Description' },
